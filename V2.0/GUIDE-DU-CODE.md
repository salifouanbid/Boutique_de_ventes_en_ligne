...
(`{ name, price, image ou tag, badge, occasion }`).
...
- Construit aussi le message WhatsApp pré-rempli spécifique à ce produit
  (avec son nom et son prix).
...
### `build()` (lignes 28-64) — la fonction principale
```js
function build() {
  ...
  // afin de pouvoir dire ensuite "ceci est nouveau" / "ceci a été supprimé"

  const products = files.map(file => {
    const { name, price, badge } = parseFilename(file);
    if (!price) { console.warn(...); }   // avertit si le prix est manquant dans le nom
    return { name: name || file, price, badge, image: `images/${file}` };
  });

  fs.writeFileSync(OUTPUT_FILE, JSON.stringify(products, null, 2));
  // réécrit entièrement images.json avec la liste actuelle des photos
...
```
À chaque exécution, le script **régénère entièrement** `images.json` à partir du
contenu actuel du dossier `images/` — il ne fait pas de mise à jour partielle.
La comparaison avec l'ancienne version sert uniquement à t'afficher un message
utile dans le terminal ("2 nouvelles photos détectées"), pas à modifier le
comportement du site.

### Mode `--watch` (lignes 68-77)
```js
...
écrasera tes modifications.

---

## 7. images/LISEZ-MOI.txt
...
une photo, même sans rouvrir ce guide.
